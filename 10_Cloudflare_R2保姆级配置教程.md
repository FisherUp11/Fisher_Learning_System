...
- 当前上传 URL 有效 10 分钟，读取 URL 有效 1 小时。单项详情里的旧链接过期后重新打开页面即可；多首歌单每次切歌和循环都会自动获取新的读取链接。
...
## 14. 多首歌曲循环播放（2026-09-09）

在 **音乐天地 → 唱一唱**（或“全部”分类）中，可以看到“选一份今天的歌单”。

1. 勾选要听的歌曲，也可以搜索名称、全选搜索结果或清空勾选。
2. 点 **开始循环播放**。歌曲按勾选顺序播放，最后一首结束后自动回到第一首；只选一首时就是单曲循环。
3. 播放中可暂停、继续、上一首、下一首，或点 **结束听歌**。
4. 需要换歌单时，重新展开“选择歌曲”，修改勾选后点 **播放所选，更新歌单**；单纯改勾选不会打断当前播放。
5. “查看本次歌单和当前歌词”可查看播放顺序与当前歌词。

只显示已分配给当前孩子、已发布并上传了音频的“唱一唱”内容。没有封面的歌曲同样可用。歌单暂存在当前页面，切换孩子、离开页面或刷新后需重新选择，不新增数据库表，也不需要运行 SQL 或新增环境变量。

听歌不会自动增加练习次数、掌握程度或奖励贴纸；亲子唱过后，仍在歌曲详情手动打卡。手机需先点击播放按钮；若系统阻止自动接播，页面会提示点击“继续播放”。锁屏或切后台能否持续播放由手机系统和浏览器决定。

技术实现：`components/music-playlist.tsx` 复用一个原生 `audio` 播放器；`/api/music/playlist-audio` 每次播放一首歌时校验登录、孩子访问权限、有效分配和歌曲发布审核状态，再跳转至新签发的 R2 临时链接。媒体内容直接从 R2 下载，不经过 Vercel 转发大文件。播放失败时会停止并提供“重试本首”及“下一首”，避免静默跳过所有失败歌曲或无限重试。
