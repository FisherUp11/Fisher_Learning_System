...
type QueryState = { learnerId: string; query: string; status: string; attempts: string; page?: number };

function libraryHref({ learnerId, query, status, attempts, page = 1 }: QueryState) {
...
export function LibraryControls({ learners, learnerId, query, status, attempts }: {
  ...
  return <>
    ...
    <form className="library-filters" onSubmit={(event) => { event.preventDefault(); applyFilters(new FormData(event.currentTarget)); }}>
      ...
      {(query || status !== "all" || attempts !== "all") && <button className="text-button" type="button" onClick={() => router.push(libraryHref({ learnerId, query: "", status: "all", attempts: "all" }))}>清除条件</button>}
    </form>
  </>;
}

export function LibraryPagination({ learnerId, query, status, attempts, page, pageCount }: QueryState & { page: number; pageCount: number }) {
  const router = useRouter();
  if (pageCount <= 1) return null;
  const goToPage = (nextPage: number) => router.push(libraryHref({ learnerId, query, status, attempts, page: nextPage }));
