...
export function MusicPracticePanel({ learnerId, itemId, itemType, title, lyrics, correctAnswer, instructions, assets }: {
  ...
  return <>
    <section className={`music-learning-card ${itemType}`}>
      ...
      {audio?.url ? <div className="music-player"><span>{itemType === "song" ? "歌曲音频" : itemType === "instrument" ? "辨音片段" : "节奏示范"}</span><audio controls preload="metadata" src={audio.url}>你的浏览器不支持音频播放。</audio></div> : <p className="notice">家长还没有上传音频；可以先阅读内容，但暂时不能播放。</p>}
