...
- **按钮反馈**：切换模块立即显示“正在打开”；导入和关键保存显示处理状态，处理中阻止重复提交。导航改为按用户意图预取目标页，减少同时加载多个模块的请求。
- **多首歌曲循环**：在“音乐天地 → 全部 / 唱一唱”勾选歌曲，按勾选顺序播放，最后一首结束后回到第一首；支持搜索、暂停、上一首、下一首和结束听歌。歌单仅使用当前孩子已分配且有音频的歌曲，离开页面会停止；听歌本身不自动打卡或发贴纸。
- **导入确认与去重**：汉字、诗词、要理问答 CSV 先确认文件与孩子，再执行导入；成功后弹窗显示结果，相同内容显示已导入。校验失败保留填写内容，自己的未完成导入可用同一文件重试。旧的重复资源不会自动删除。

本轮保持现有学习规则与数据表不变，更新代码并重新部署即可，无需新增 SQL 或环境变量。更新后的验收步骤见 [部署说明](./DEPLOYMENT.md#52-2026-09-按钮歌单与导入优化验收)。
