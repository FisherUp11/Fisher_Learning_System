...
> 目标：先为一位孩子做出一套每天愿意打开、能持续复习、父母看得懂进度的学习工具；目前已包含识字、诗词背诵、音乐练习、儿童信仰问答与小芽贴纸奖励五个模块。

## 最初 MVP 决策（历史背景）

项目最初只做 **「汉字识认 + 拼音辅助 + 间隔复习 + 家长导入/看进度」**。在这一闭环稳定后，现已按独立数据边界加入诗词、音乐和儿童信仰问答；录音自动评分与开放式 AI 对话仍不在当前范围。
...
本目录同时是 MVP 应用根目录：`app/` 是 Next.js 前端；`supabase/001_hanzi_mvp.sql` 是识字基础表、RLS 和复习函数；`004`–`007` 是早期识字升级；`008`–`012` 分别加入诗词、音乐、信仰问答、重点字和小芽贴纸；`013`–`014` 完成队列歧义修复、动态双确认和同日重试；`015` 新增学习空间、家庭、角色、资源审核/分配和安全邀请；`016` 将学习 RPC 接到新权限边界并增加汉字有界自适应复习；`017` 增加 owner 用户目录、首次改密和重复资源安全合并。最新权限说明见 [15 号说明](./15_多家庭管理员与智能复习说明.md) 与 [16 号说明](./16_用户家庭管理与资源安全清理说明.md)。密码恢复使用 Supabase Auth + Resend SMTP，按 [17 号配置教程](./17_Resend密码恢复邮件配置教程.md) 操作；音乐文件放在私有 Cloudflare R2，请严格按 [DEPLOYMENT.md](./DEPLOYMENT.md) 与 [R2 配置教程](./10_Cloudflare_R2保姆级配置教程.md) 操作。
