...
本目录同时是 MVP 应用根目录：`app/` 是 Next.js 前端；`supabase/001_hanzi_mvp.sql` 是识字基础表、RLS 和复习函数；`004`–`007` 是后续识字升级；`008` 新增诗词背诵记录；`009` 新增音乐内容、R2 媒体元数据、练习历史与记忆阶段；`010` 新增儿童信仰问答、双语内容、人工判断历史和复习 RPC；`011` 新增孩子级重点字、字库筛选统计和优先队列；`012` 新增小芽贴纸、成长星、礼物与兑换流水；`013` 修复跨日队列字段歧义；`014` 新增动态双确认、柔和降级和不限次数的当天队尾重试。汉字最新规则见 [动态双确认说明](./14_汉字动态双确认规则说明.md)。音乐文件放在私有 Cloudflare R2，请严格按 [DEPLOYMENT.md](./DEPLOYMENT.md) 与 [R2 配置教程](./10_Cloudflare_R2保姆级配置教程.md) 操作。
