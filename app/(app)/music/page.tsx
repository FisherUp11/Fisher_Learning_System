...
export default async function MusicPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div>
    ...
    {items.length === 0 ? <section className="empty panel"><span className="empty-mark">♫</span><h2>{learner.display_name} 还没有音乐内容</h2><p className="lede">管理员分配已发布的音乐内容后，就会出现在这里。</p>{access?.isAdmin && <Link className="primary" href="/music/manage">去音乐内容工作台</Link>}</section> : <>
      ...
      {(selectedType === "all" || selectedType === "song") && <MusicPlaylist key={learner.id} learnerId={learner.id} songs={items.filter((item) => item.itemType === "song" && item.assets.some((asset) => asset.asset_type === "audio")).map((item) => ({ id: item.id, title: item.title, category: item.category, lyrics: item.lyrics }))} />}
