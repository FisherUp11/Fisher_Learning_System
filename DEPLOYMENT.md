...
**2026-09 操作体验更新**：本轮增加即时按钮反馈、多首歌曲循环播放、三类 CSV 导入确认与相同内容检查。已经正常运行当前版本的项目只需更新代码并重新部署，不用新增 SQL、环境变量、R2 Bucket 或 Supabase Function。现有重复资源不会自动删除。升级后按 [5.2 节](#52-2026-09-按钮歌单与导入优化验收) 测试。
...
24. 如果已有历史重复字册，在“管理中心 → 资源”选择一份保留并安全合并；确认孩子分配、学习次数和阶段不变。新版本重复导入相同内容会提示已导入，不再通过再次上传来制造重复册。给测试音乐先产生一条练习历史，再尝试合并删除，系统必须拒绝并提示归档。
...
### 5.2 2026-09 按钮、歌单与导入优化验收

1. 本地运行最新代码，或完成 Vercel 新部署后刷新页面。点“芽 / 册 / 家”和顶部学习模块，较慢请求时应立即显示“正在打开”；同一按钮等待期间反复点击不会重复发起该导航。用键盘 Tab 切换焦点、Esc 关闭模块菜单，确认入口可正常使用。
2. 在“家长”页修改孩子设置并保存，确认出现处理状态和保存成功弹窗；处理期间不能再次提交。设置保存完成后检查页面中的新值。
3. 分别选择一个有效的汉字、诗词、问答 CSV。点击导入时先出现确认框，核对文件、名称与孩子；点“再检查一下”应不导入，输入和已选文件保留。
4. 再次确认导入，等待过程中按钮不可重复提交；完成后弹窗显示成功或待审核信息。关闭后到对应模块或资源管理核对内容。发布/审核与分配仍沿用原有管理员规则。
5. 在同一账号重新选择完全相同的 CSV，即使改了文件名，也应提示已有相同内容，不新增第二份资源。校验错误时保留表单；网络不确定时用同一文件重试，系统会检查已存在的内容。历史半成品无法自动恢复时按提示交由管理员检查。
6. 进入“音乐天地 → 全部 / 唱一唱”，确认能看到当前孩子已分配且有音频的歌曲。按希望的播放顺序勾选 2–3 首，点“开始循环播放”；听到最后一首结束后应自动回到第一首。
7. 测试暂停、继续、上一首、下一首、结束听歌；通过搜索选歌。播放期间修改勾选不会立即打断当前歌单，点击“播放所选，更新歌单”后才生效。离开页面或切换孩子应停止当前播放。
8. 在 iPhone Safari 上手动点一次开始播放；如果系统阻止接续播放，点“继续播放”。锁屏/后台持续播放由浏览器与系统控制。纯听歌不自动增加练习次数、记忆阶段或贴纸；一起唱过后仍到歌曲详情主动打卡。

如果某首歌无法播放，先尝试“重试本首”或“下一首”；再核对该内容是否仍已发布、已审核、已分配给当前孩子，以及音频是否已上传。多歌循环继续使用已有私有 R2 配置，不需要将 Bucket 改为公开。
