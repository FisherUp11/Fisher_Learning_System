...
| Supabase Storage | 私有朗读缓存、家长上传 MP3/封面/录音 | 不创建公开可猜测的儿童音频链接 |
...
| 表 | 用途 |
| --- | --- |
| `poems`, `poem_lines` | 诗题、作者、朝代、原文及分句。 |
| `music_tracks`, `music_segments` | 家长导入的 MP3、封面、歌词/诗歌段落。 |
| `recitation_attempts` | 后续背诵自评/语音识别的结果，默认与普通学习尝试分开。 |

建议让 `item_type` 先支持 `character`；等古诗上线后再添加 `poem`，不要提前为了“通用性”建立过度抽象的十几张表。
...
| `family-media` | 私有 | 家长导入 MP3、封面 | 仅所属家长写/读；孩子只拿可播放签名 URL |
| `recitations`（后续） | 私有 | 孩子录音 | 默认仅家长；支持删除/自动过期 |

文件路径包含家庭/内容归属，例如 `families/{parent_user_id}/music/{track_id}/original.mp3`。实现时用 `storage.objects` 的 RLS 策略限制路径前缀，不使用公共 bucket。
