...
| Supabase Postgres | 内容、当前学习状态、不可变尝试记录、调度、贴纸流水与兑换 | 不保存 Azure 密钥 |
...
| `learner_content_packages` | `learner_id, package_id, linked_at` | 孩子与历次导入字册的长期关联。 |
| `learner_character_priorities` | `learner_id, character_id, selected_at` | 孩子级重点字；同一个字可对不同孩子有不同优先级。 |
...
| `poem_game_maps`, `poem_game_sessions`, `poem_game_attempts` | 可缓存诗意地图、一局汇总与每次游戏选择事实。 |
| `learner_poem_line_states` | 独立于整首诗评分的逐句熟练度和下次建议复习时间。 |
...
| `reward_accounts` | 每个孩子的成长星设置和当前进度；不作为贴纸余额来源。 |
| `reward_ledger` | 获得、消费和返还贴纸的不可变流水；余额由 `amount` 求和。 |
| `reward_growth_events` | 诗词/音乐某项目某天是否获得成长星及每日上限原因。 |
| `reward_catalog_items`, `reward_redemptions` | 家长礼物愿望、兑换快照和可撤销状态。 |

汉字、诗词、音乐和奖励保持各自清晰的状态/历史表；不为了“通用性”把不同语义塞进一张充满 nullable 字段的表。奖励只引用原练习记录，不能反向修改学习阶段。
...
4. 按“重点字优先、`due_at` 更早优先、阶段更低优先”选择到期项目；重点字仍须到期，不能提前复习。
5. 新字名额先从孩子全部已关联字册选未学重点字，再从当前激活字册按 `package_characters.sequence` 补普通新字；两者共同受每日新字上限约束。
...
8. 当汉字真实待答数归零时，按孩子本地日期幂等发放每日贴纸；诗词/音乐完成后从不可变练习记录反查项目和结果，再登记成长星。
...
- 使用数据库索引：`learning_states(learner_id, due_at)`、`learning_attempts(learner_id, item_id, answered_at desc)`、`package_characters(package_id, sequence)`、`learner_character_priorities(learner_id, selected_at, character_id)`。
